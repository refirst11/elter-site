...
## Q&A
### Compile error
compiler uses fs to build css, so it cannot parse imports such as .css and .svg and other image file extention. To deal with this issue, load images via src="".  
Do not use simultaneously on css and tsx (separate into ts files, or delete the css import)  

cssx only compiles ts or tsx files.  
...
## The name may change soon
### TypedCSSX
This name is certainly stiff, long, and difficult to remember.
However, we initially decided on it because we thought it was the best name that clearly expressed the technical content, but we fully intend to change it in the future.