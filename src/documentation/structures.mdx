...
# TypedCSSX Structures

## Ease of introduction
The TypedCSSX mechanism scans and compiles cssx method descriptions using an AST (Abstract Syntax Tree).
This allows it to operate in a Node.js environment where JavaScript can be executed.
This means it's not dependent on any specific framework.

### Concise API
The learning curve is very gentle.
It works with zero configuration in Next.js by default.
All you need to do is compile with npx typedcssx --compile and
introduce the optimization plugin.

### Simple methods
To be able to use the existing className API, the main methods create and set only return strings.
The rest are just the root method for describing variables at the root, and the global method for describing globals.
...
### Complete static determinism
In TypedCSSX, styles are completely statically determined.
This means that CSS is completely statically determined at compile time, providing a deterministic and fully static CSS solution.
You can create maintainable, scalable, deterministic, and easy-to-manage CSS.
...
This means that even as a result of natural description, the style object bundle doesn't increase, and only lightweight CSS remains.
...
The design of using TypedCSSX objects as hashes for IDs results in collision drawing at the function level,
which dramatically reduces the number of draws.
For example, if you're converting strings to IDs at the class name level, collisions can occur even within functions.
This means that the number of draws increases each time a class name is created.
...
This cannot be achieved with a design that generates random hashes with each compilation.
